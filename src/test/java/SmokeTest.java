...
public class SmokeTest {
    ...
    public void calculatorIKTTest() {
        driver.get("https://clinic-cvetkov.ru/company/kalkulyator-imt/");//открываем ссылку
        System.out.println(driver.getCurrentUrl());//возвр текущее значение урлы (которое на экране)
        System.out.println(driver.getTitle());//возврщаает тайтл тек страницы. (инфа на закладке)
        //getPageSource - вернет html док страницы

        WebElement weightInput = driver.findElement(By.name("weight"));//1 элемент
        WebElement heightInput = driver.findElement(By.name("height"));
        WebElement calcButton = driver.findElement(By.id("calc-mass-c"));

        List<WebElement> buttonsList = driver.findElements(By.tagName("button"));//список элементов
